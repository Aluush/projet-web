...
include('db_cand.php');
    ...
    $gmail="SELECT gmail_cand FROM candidats WHERE gmail_cand='$gmail_cand'";
    $prep=$conn->prepare($gmail);
 $prep->execute();
 $execute=$prep->fetchAll();

    
    if(!empty($execute)){ 
      header("Location:signup_candidat.php?msg='l'email deja existe!!'");
    }
    else{ 
